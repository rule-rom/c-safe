...
│   └── echo.clj          # Генератор Markdown-отчетов (Зеркало)
├── specs/                # Локальные прототипы интентов (EDN)
│   ├── c_core.edn
│   ├── memory-safety.edn
│   └── resource_guard.edn
├── scripts/              # Утилиты сборки
├── MANIFEST.md           # Манифест Семантического Якоря
└── CONTRACT.md           # Контракт для ИИ-агентов
```
...
|-------------|------------|
| [`intent-garden/core`](https://github.com/intent-garden/core) | Clojure-движок валидации |
| [`rulerom/registry`](https://github.com/rule-rom/registry) | Глобальная библиотека Intent-контрактов (Git Submodule) |
